...
def train_and_validate(rank, world_size, config):
    ...
    model = PoseGCNTransformer(
        ...
        in_channels=3,
        hidden_dim=160,  # 更高的隐藏维度
        num_classes=config['num_classes'],
        num_gcn_layers=2,  # 增加GCN层数
        num_transformer_layers=8,  # 更多Transformer层
        feat_dim=64,  # 特征维度
    ...
    #     if rank==0:
    #         print(f"loading model weights from {config['save_path']}")
    ...
    train_dataset = H5PoseDataset(
        ...
        num_joints= 23    #jin shi yong shen ti da guan jie
    )
    ...
    for epoch in range(config['num_epochs']):
        ...
        if rank == 0:
            ...
            if val_acc > best_val_acc:
                ...
                patience_counter += 1
                print(f"Validation accuracy did not improve, Patience:{patience_counter}/{config['patience']}")

            if patience_counter >= config['patience']:
                print(f"Early stop after {epoch +1} epoches")
                break
...
if __name__ == "__main__":
    ...
    config = {
        ...
        'save_path': 'checkpoints/64-77.pth',  # 模型保存路径
        'reverse_prob': 0.8,  # 倒序概率
        'mask_ratio': 0.2,  # 时间掩码比例
        'noise_scale': 0.02,  # 噪声强度
        'patience':10  # zao ting
    }
